...
class AppStore extends Component {
  ...
  filteredApps = () => {
    ...
    const filteredList1 = appsList.filter(each => each.category === tabIds)
  ...
  render() {
    const filteredList1 = this.filteredApps()
    const {tabIds} = this.state
    const {inputSearch} = this.state
    const inputSearchedList = filteredList1.filter(each =>
    ...
    return (
      <div className="bg-image">
        ...
        <ul className="for-tab">
          {tabsList.map(each => (
            <TabItem
              each={each}
              key={each.tabId}
              onChangesApp={this.onChangesApp}
              isButtonActive={each.tabId === tabIds}
            />
          ))}
        </ul>
        <ul className="for-App">
          {inputSearchedList.map(each => (
